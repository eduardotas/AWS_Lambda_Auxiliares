...
from processors import (setor_brick, aux_state_uf, pdv_cluster, setor_cnpj_acesso,
                        filtro_ra_proprio_concorrente, grade_produtos, tabela_mercado,tabela_geral_mercado,
                        mercado_classe_nps_gps, painel_hcp,de_para_ponderacao,asma)

s3 = boto3.client('s3')

def lambda_handler(event, context):
    print('Starting az-latam-brazil-auxiliares-im-s3-to-parquet')
    print(event)
    ...
    try:
        ...
        for item in items:
            ...
            elif 'filtro_ra_proprio-concorrente' in key.lower():
