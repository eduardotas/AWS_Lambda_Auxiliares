...
def process(s3, file_key):
    ...
    try:
        ...
        ## Criando a tabela
        sql = f'CREATE TABLE IF NOT EXISTS {constants.rs_schema}.{redshift["table"]} (id_pdv varchar (50), outlet_cd varchar (50), cluster varchar (250), channel_pvpb	varchar (50), channel_group	varchar (50), load_data timestamp);'
        engine.execute(sql)
        ...
        s3_utils.archive_file(s3, 'pdv_cluster', filename, file_key)
