...
def process(s3, file_key):
    ...
    redshift = {
        ...
        'table': 'aux_sector_cnpj_access_tb'
    }
    ...
    try:
        ...
        ## Criando a tabela
        sql = f'CREATE TABLE IF NOT EXISTS {constants.rs_schema}.{redshift["table"]} (id_store int, id_set int, cluster int, factor varchar(100), load_data timestamp)'
        engine.execute(sql)
        ...
        print(df.columns)
        df = df.rename({'store_id':'id_store', 'set_id':'id_set', 'cluster':'cluster', 'factor':'factor', 'load_data':'load_data'}, axis=1)
        ...
        s3_utils.archive_file(s3, 'setor_cnpj_acesso', filename, file_key)
