...
def process(s3, file_key):
    ...
    try:
        ...
        ## Criando a tabela
        sql = f'CREATE TABLE IF NOT EXISTS {constants.rs_schema}.{redshift["table"]} (UNT_DESC varchar(255), SET_ID varchar(255), CRM varchar(255), NOME varchar(255), ENDERECO varchar(255), CEP varchar(255), BRICK varchar(255), FATOR varchar(255), load_data timestamp)'
        engine.execute(sql)
        ...
        s3_utils.archive_file(s3, 'painel_hcp', filename, file_key)
